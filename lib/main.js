...
var pTitleItem = true, purlItem = true, pHostnameItem = true, pTextItem = true, aHtmlItem = true, hCodeItem = true, lTextItem = true, lHostnameItem = true, textTitleHotkey = true, purlHotkey = true;
function refresh_addon() {
	...
	if(db.storage.options.cAsHtml && aHtmlItem) {
		asHtmlItem = contextMenu.Item({
		  label: "Copy as HTML",
		  context: contextMenu.SelectionContext(),
		  contentScript: 'self.on("click", function (node, data) {' +
				         '  self.postMessage("");' +
		  ...
		  onMessage: function () {
		  			if(selection.html) {
						clipboard.set(selection.html.trim(), 'html');
					}
		  }
		});
		aHtmlItem = false;
	} else if(!db.storage.options.cAsHtml && !aHtmlItem) {
		asHtmlItem.destroy();
		aHtmlItem = true;
	}
