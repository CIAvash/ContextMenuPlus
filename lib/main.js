...
**/
var prefList = ['copyPageTitle', 'copyPageURL', 'copyPageTitleAndURL', 'copyTitleAndURLOfAllTabs', 'copyAsHTMLLink', 'copyAsLink', 'openPageHostname', 'copyAsPlainText', 'copySelectionAsHTMLLink', 'copySelectionAsLink', 'copyAsHTML', 'copyTextAndURL', 'copyLinkText', 'openLinkHostname', 'stopLoadingImage', 'copyFeedURL', 'ptextAndPtitleHotkey', 'copyPageURLHotkey'];

//recovering preferences because of Add-on SDK 1.4 bug
if(!prefs.prefs.recovered) {
    for(var i=0; i<prefList.length; i++) {
        if(typeof recovery.prefs[prefList[i]] != 'undefined') {
            prefs.prefs[prefList[i]] = recovery.prefs[prefList[i]];
        }
    }
    prefs.prefs.recovered = true;
}
