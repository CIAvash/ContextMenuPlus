...
function ptextAndPtitleHotkey(prefName) {
	var hotkey = getHotkey(prefName);
	if(!/[a-zA-Z0-9]/.test(hotkey) || hotkey === prefs.prefs['copyPageURLHotkey']) hotkey = '';
...
function copyPageURLHotkey(prefName) {
	var hotkey = getHotkey(prefName);
	if(!/[a-zA-Z0-9]/.test(hotkey) || hotkey === prefs.prefs['ptextAndPtitleHotkey']) hotkey = '';
	prefs.prefs[prefName] = hotkey;
	db.storage.hotkeys[prefName] = hotkey;
	if(hotkey) {
