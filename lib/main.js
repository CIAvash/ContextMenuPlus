...
var pTitleItem = true, purlItem = true, ptURLItem = true, tURLTabsItem = true, phLinkItem = true, paLinkItem = true, pHostnameItem = true, pTextItem = true, thLinkItem = true, taLinkItem = true, hCodeItem = true, stURLItem = true, lTextItem = true, lHostnameItem = true, textTitleHotkey = true, purlHotkey = true;
function refresh_addon() {
	...
	if(db.storage.options.cpTitleURL && ptURLItem) {
		pTitleURLItem = contextMenu.Item({
		...
		ptURLItem= false;
	} else if(!db.storage.options.cpTitleURL && !ptURLItem) {
		pTitleURLItem.destroy();
		ptURLItem = true;
	}
	if(db.storage.options.cTitleURLTabs && tURLTabsItem) {
		titleURLTabsItem = contextMenu.Item({
		...
		tURLTabsItem= false;
	} else if(!db.storage.options.cTitleURLTabs && !tURLTabsItem) {
		titleURLTabsItem.destroy();
		tURLTabsItem = true;
	}
	...
	if(db.storage.options.ctAsHtmlLink && thLinkItem) {
		tHtmlLinkItem = contextMenu.Item({
		  label: "Copy as HTML Link",
		  context: contextMenu.SelectionContext(),
		  contentScript: 'self.on("click", function (node, data) {' +
		  				 '	if(document.URL) {' +
		  				 "		var link = '<a href=\"' + document.URL + '\">' + window.getSelection().toString() + '</a>';" +
				         '  	self.postMessage(link);' +
				         '}});',
		  onMessage: function (link) {
					clipboard.set(link);
		...
		thLinkItem = false;
	} else if(!db.storage.options.ctAsHtmlLink && !thLinkItem) {
		tHtmlLinkItem.destroy();
		thLinkItem = true;
	}
	if(db.storage.options.ctAsLink && taLinkItem) {
		tAsLinkItem = contextMenu.Item({
		  label: "Copy as Link",
		  context: contextMenu.SelectionContext(),
		  contentScript: 'self.on("click", function (node, data) {' +
		  				 '	if(document.URL) {' +
		...
		taLinkItem= false;
	} else if(!db.storage.options.ctAsLink && !taLinkItem) {
		tAsLinkItem.destroy();
		taLinkItem = true;
	}
	...
	if(db.storage.options.csTextURL && stURLItem) {
		sTextURLItem = contextMenu.Item({
		  label: "Copy Text and URL",
		  context: contextMenu.SelectionContext(),
		  contentScript: 'self.on("click", function (node, data) {' +
		  				 '	if(document.URL) {' +
		  				 '		var textURL = window.getSelection().toString() + "\\r\\n" + document.URL;' +
		...
		stURLItem = false;
	} else if(!db.storage.options.csTextURL && !stURLItem) {
		sTextURLItem.destroy();
		stURLItem = true;
	}
